...
export interface SaveBenefitResult {
  success: boolean;
  error?: string;
}

/**
 * Persist a benefit (and its related contacts/documents/disclaimers) for the
 * plan/category described by the wizard `stepData`.
 *
 * Extracted from the original `submitBenefits()` in the Create Benefit wizard so
 * the wizard and the inline "Edit Benefit" page share one merge implementation.
 * Callers own UI concerns (toasts, step navigation, loading state).
 */
export async function saveBenefit(
  stepData: BenefitsWizardState["stepData"],
): Promise<SaveBenefitResult> {
  const step1Data = stepData.step1;
  const step3Data = stepData.step3;
  const step4Data = stepData.step4;
  const step5Data = stepData.step5;
  ...
  if (!planId) {
    return { success: false, error: "Plan ID missing. Cannot complete setup." };
  }
  ...
  try {
    ...
    const newContactsToAdd = localContacts.filter((lc: any) =>
      !existingContacts.some((ec: any) => ec.id === lc.id || (lc.email === ec.email && ec.email))
    );
    ...
    return { success: false, error: error?.message || "Failed to save benefit" };
  }
}
